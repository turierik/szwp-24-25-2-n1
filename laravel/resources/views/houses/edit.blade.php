...
<form method="POST" enctype="multipart/form-data" action="{{ route('houses.update', ["house" => $house]) }}">
    ...
    <select name="owner_id">
        @foreach($users as $user)
            <option value="{{ $user -> id}}" {{ old('owner_id', $house -> owner_id) == $user -> id ? 'selected' : '' }}>{{ $user -> name }}</option>
        @endforeach
    </select>
    @error('owner_id')
       <span class="text-red-500 font-medium">{{ $message }}</span>
    ...
    Ár:
    <input type="text" name="rent" value="{{ old('rent', $house -> rent) }}">
    @error('rent')
       <span class="text-red-500 font-medium">{{ $message }}</span>
    @enderror<br>

    Méret:
    <input type="text" name="size" value="{{ old('size', $house -> size) }}">
    @error('size')
       <span class="text-red-500 font-medium">{{ $message }}</span>
    @enderror<br>
